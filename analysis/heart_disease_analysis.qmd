...
results_df = pd.read_csv("../results/evaluate_model_results.csv")
...
Markdown(heart_data.head().to_markdown(index=False))
```
...
![Class balance of the heart disease target variable (Presence vs. Absence).](../results/eda_results/target_distribution.png){#fig-targ-dist width=60%}
...
cv_results = pd.read_csv("../results/CV_scores_default_parameters.csv")
Markdown(cv_results.round(3).to_markdown(index=False))
```
...
hyp_df = hyp_df.rename(columns={hyp_df.columns[0]: "Model"})
Markdown(hyp_df.round(4).to_markdown(index=False))
```
