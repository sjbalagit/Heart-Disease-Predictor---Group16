...
![Distribution of Target Variable.](../results/target_distribution.png){#fig-targ-dist width=60%}
...
![Distribution of Numerical Features.](../results/numerical_feature_distributions.png){#fig-num-hist width=60%}
...
![Boxplots of Numeric Features.](../results/boxplots_vs_target.png){#fig-num-box width=60%}
...
![Distribution of Categorical Features.](../results/categorical_vs_target.png){#fig-cat-dist width=60%}
...
![Confusion matrix of SVM RBF performance on test data.](../results/.png){#fig-conf-mtx width=60%}

# 4. Results and Discussion

**Model Perforamce**

After extensive preprocessing and EDA, we identified our best performing model among the three candidates - Decision Tree Classifier, Logistic Regression and Support Vector Machine (SVM) with Radial Basis Function (RBF) Kernel and Dummy Classifier as the baseline by using cross-validation and randomized search of hyperparameters. SVM RBF with hyperparameters C = 1 and $ \gamma $ (Gamma) = 0.1 stands out as the best perfomer achieving a validation score of **inline code 0.9646** - higher than the other two models. The model achieved a score of 0.98 when deployed on the test data - outperforming the validation score we achieved. It successfully minimized the False Negatives with **inline code Recall = 1.0 and Precision = 0.97** as seen in the @fig-conf-mtx.
...
Given the high stakes of medical diagnosis, our priority was to minimize False Negatives—ensuring that patients with heart disease are not incorrectly classified as healthy. Our model succeeded in this objective, achieving a perfect Recall of 1.0 on the test set. This means the model successfully flagged every positive case of heart disease in the unseen data. While there were a small number of False Positives (Precision), this is an acceptable trade-off in a screening context, as the cost of a follow-up test is far lower than the cost of a missed diagnosis.
