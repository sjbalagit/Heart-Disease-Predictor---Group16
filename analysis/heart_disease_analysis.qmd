...
svm_row = hyp_df[hyp_df.iloc[:, 0] == 'SVM RBF'].iloc[0]
best_val_score = round(svm_row['F2 Score'], 4)
params = ast.literal_eval(svm_row['Best Model Parameters'])
best_c = params.get('svc__C')
best_gamma = params.get('svc__gamma')
...
![Class balance of the heart disease target variable (Presence vs. Absence).](../results/eda_results/target_distribution.png){#fig-targ-dist width=85%}

**Feature Distributions:** As seen in @fig-num-hist, none of the continuous features follow a perfect normal distribution. Notably, serum cholesterol and oldpeak are right-skewed, while maximum heart rate is left-skewed. Oldpeak represents how much the heart’s electrical activity shows stress related oxygen deprivation during exercise. Larger values indicate worse blood flow to the heart muscle.

![Histograms displaying the distribution of continuous features.](../results/eda_results/numerical_feature_distributions.png){#fig-num-hist width=85%}
...
![Distribution of numeric features stratified by heart disease diagnosis.](../results/eda_results/boxplots_vs_target.png){#fig-num-box width=85%}
...
![Prevalence of heart disease across gender, chest pain type, and other categorical variables.](../results/eda_results/categorical_vs_target.png){#fig-cat-dist width=85%}
...
![Heatmap displaying Pearson correlation between predictors and target in heart disease dataset.](../results/eda_results/correlation_heatmap.png){#fig-corr-map width=85%}
...
#| tbl-cap: 5 fold cross-validation scores and time (in seconds) for different models with default parameters.
